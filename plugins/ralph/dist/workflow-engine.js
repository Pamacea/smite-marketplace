...
class WorkflowEngine {
    constructor(options) {
        this.configManager = new workflow_config_1.WorkflowConfigManager();
        this.smiteDir = options.smiteDir;
        this.mcpEnabled = options.mcpEnabled ?? true;
        this.specGenerator = new spec_generator_1.SpecGenerator(options.smiteDir);
    }
    async executeWorkflow(story, workflowId, options = {}) {
    ...
    initializeWorkflowState(workflow, storyId) {
        return {
            workflowId: workflow.id,
            currentStep: workflow.steps[0]?.id || "analyze",
            completedSteps: [],
            failedSteps: [],
            stepResults: new Map(),
            startTime: Date.now(),
    ...
    async executeStep(story, step, options) {
    ...
    async executeAnalyzeStep(story, step, mcpCalls) {
    ...
    async executePlanStep(story, step) {
    ...
    async executeResolveStep(story, step) {
        return `Resolution applied for ${story.id}`;
    }
    async executeVerifyStep(story, step, mcpCalls) {
        if (step.mcpTools?.includes("toolkit") && this.mcpEnabled) {
    ...
    async executeCompleteStep(story, step) {
